...
uv run python ablation_kitti-c.py --ablations overnight \
    ...
    --log_dir logs/diagnostics_overnight/methods | tee logs/diagnostics_overnight_methods.log
...
uv run python ablation_kitti-c.py --ablations d3_sweep \
    ...
    --chunked --reset_per_corruption \
    --log_dir logs/diagnostics_overnight/d3 | tee logs/diagnostics_overnight_d3.log
...
uv run python ablation_kitti-c.py --ablations m_d_prior_switch,m_a_adaptive_cap \
    ...
    --chunked --reset_per_corruption \
    --log_dir logs/diagnostics_overnight/d5 | tee logs/diagnostics_overnight_d5.log
...
uv run python ablation_kitti-c.py --ablations legacy_val \
    ...
    --corruptions $ALL_CORRUPTIONS \
    --chunked --reset_per_corruption \
    --log_dir logs/diagnostics_overnight/legacy_chunked | tee logs/diagnostics_overnight_legacy_chunked.log
...
uv run python ablation_kitti-c.py --ablations legacy_loose_t1 \
    ...
    --continual \
    --log_dir logs/diagnostics_overnight/legacy_continual | tee logs/diagnostics_overnight_legacy_continual.log

...
echo "=== [full-dataset] $SUFFIX [$CONDS] on cov-shift ep10 + DGLSS++ ==="
CUDA_VISIBLE_DEVICES=$GPU uv run python robust_diagnostic/al_full_dataset_diag.py \
  --label "full_${SUFFIX}" \
  --conds "$CONDS" --max_frames "$MAX_FRAMES" \
  --extractors \
    "cov_ep10:${METHOD}:${CKPT},dglsspp:supcon_vib_dglsspp:${DGLSSPP_CKPT}" \
...
if [ $RC -eq 0 ]; then
  ...
  echo "  - for BOTH cov-shift ep10 and DGLSS++ extractors"
else

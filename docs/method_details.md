...
**Failure 2 - Raw geometric arguments fail in hyperspace due to class imbalance (Voronoi Shattering).** In a 10,000D hyperdimensional space, the concentration of measure compresses almost all cosine similarities into a microscopically narrow mathematical band. Because geometric distances are so tightly compressed, the volume of a class's decision boundary (its Voronoi cell) becomes highly brittle and exquisitely sensitive to the number of points updating it. If adaptation relies on passive geometric arguments (e.g., standard pseudo-labeling, spatial gating, or distance thresholds), it cannot distinguish between a legitimate structural domain shift and massive Out-Of-Distribution noise. Consequently, the avalanche of confidently wrong majority class points (like Road or Building on wet ground) slightly rotates their prototypes, which in 10,000D space completely engulfs and shatters the tiny boundaries of rare classes. On wet ground, the −7.2 collapse is precisely this: majority prototypes rotate by tens of degrees and cannibalize the tail classes. A geometric confidence gate does not prevent this, selection quality is the wrong control surface. **The correct surface is an explicit physics bound on prototype displacement (a rotation cap)**, which this method imposes.

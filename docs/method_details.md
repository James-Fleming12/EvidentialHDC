...
## 3. Network Uncertainty (Epistemic Density)

### 3.1 The Method: Dirichlet Density Estimation
...
Points with $u > 0.5$ trigger the **Epistemic Veto**, which zeros out the gradients for that instance.

### 3.2 Test Results
In isolation, Epistemic Density proved to be an exceptionally strong universal gate. It yielded significant improvements across structured corruptions (e.g., **+8.53% mIoU on Snow** over the frozen baseline) while uniquely providing robust protection against chaotic noise (+0.99% on Fog).

### 3.3 Comparison to SOTA (D3CTTA)
Standard SOTA architectures like D3CTTA rely on entropy minimization. Under severe semantic corruption, entropy minimization blindly enforces confidence on inherently noisy vectors. This triggers catastrophic confirmation bias and mode collapse. By actively measuring Epistemic Density rather than just entropy, Evidential HDC completely shatters D3CTTA’s performance, doubling or tripling the mIoU across multiple corruptions.

---

## 4. HDC Uncertainty (Geometric / Latent Density)

### 4.1 The Method: Latent Geometric Analysis (Free Energy)
HDC provides a fixed $D$-dimensional hypersphere where semantic relationships are physically encoded by angular distances. Geometric uncertainty measures the dispersion of a point within this space independent of the network's final predictive logits. We quantify this using the Free Energy of the latent geometry:
$$ F(z) = -T \log \sum_{c=1}^C \exp\left(\frac{S(z, \tilde{w}_c)}{T}\right) $$
where $T$ is the temperature scaling. By evaluating the exponential sum of cosine similarities across all prototypes simultaneously, the model measures the latent geometric density of the point. A point with low Free Energy is geometrically well-anchored within the dense regions of the known class manifolds, while high Free Energy indicates semantic isolation (e.g., unstructured fog noise). Points exceeding a geometric density threshold are gated out of the adaptation step.

### 4.2 Rejected Method: The Ensemble Paradox
**The Attempt:** We hypothesized that ensembling orthogonal uncertainty metrics (Network Epistemic + HDC Geometric + Temporal) into a single logical AND gate would yield the ultimate robust filter.
**Why it Failed:** Over-gating triggered severe **Representation Shrinkage**. The intersection of three strict filters dropped the gradient admission rate from ~70% to 20%. Because the model was vetoing all diverse, edge-case, and hard examples, the prototypes shrank into hyper-dense geometric cores. They lost coverage over the full manifold of the class, severely degrading performance on complex corruptions like Wet Ground and Beam Missing. A single, well-calibrated metric (Epistemic Density) proved mathematically superior for maintaining manifold coverage.

---

## 5. Inter/Intra Class Balance

### 5.1 The Method: Explicit Calibrated Prior & IC4
...
* **IC1 (Hard Rotation Budget):** Attempted to restrict updates to a hard $5^\circ$ angular cap per chunk. It failed to change performance because Bayesian Momentum (see Section 6) inherently constrained rotations to $<4.5^\circ$ organically.
...
### 6.1 The Method: Bayesian Momentum

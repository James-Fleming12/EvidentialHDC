...
**LP-pool anomaly (verified with the self-check, still unexplained)**: the self-check confirmed the LP classifier is correct (LP accuracy on the val subset is 36.8% on fog and 35.1% on crosstalk, matching the autopsy headroom exactly), yet on the *pool the re-estimate actually uses*, LP accuracy collapses to 12.8% / 5.8%, despite pool and val having identical class-0 fractions. Since the classifier and feature tensor are identical for both subsets, this points to a pool-vs-val composition difference that class-0 alone does not capture (per-class counts and feature-norm means of pool vs val have been added to the self-check to resolve it). The LP-pseudo re-estimate result stands as measured on the pool, but its interpretation is pending this resolution.

**Pending**: re-run `--iter1_pseudo_refine --corruptions fog,crosstalk` with the expanded self-check (per-class counts + norm means of pool vs val) to decide whether the low pool LP accuracy is a pool-composition artifact or a genuine property of the re-estimate pool.
